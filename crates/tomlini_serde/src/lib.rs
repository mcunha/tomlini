...
impl From<tomlini::ParseError> for Error {
    fn from(e: tomlini::ParseError) -> Self {
        Error::Parse(e)
    }
}
// ============================================================
...
impl<'a> serde::Serializer for &'a mut Serializer {
    ...
    fn serialize_bytes(self, _v: &[u8]) -> Result<(), Error> {
        Err(Error::Message("bytes not supported".into()))
    }
    ...
    fn serialize_tuple_variant(
        ...
        _variant: &'static str,
        _len: usize,
    ) -> Result<serde::ser::Impossible<(), Error>, Error> {
        Err(Error::Message("not supported".into()))
    }
...
pub fn from_str<T: for<'de> Deserialize<'de>>(input: &str) -> Result<(FlatDoc, T), Error> {
    let doc = tomlini::parse(input)?;
    let val = from_doc(&doc)?;
    Ok((doc, val))
}
...
// ---- types ----
// ---- types ----
struct TableEntry<'de> {
    path: Vec<&'de str>,
    keys: Vec<KeyEntry<'de>>,
    aot_entries: Vec<Vec<KeyEntry<'de>>>,
}
#[derive(Clone, Copy)]
struct KeyEntry<'de> {
    key: &'de str,
...
enum State<'de> {
    ...
    Array {
        elements: Vec<(usize, SpanKind)>,
    },
    Inline {
        pairs: Vec<(KeyEntry<'de>, usize)>,
    },
    AoT {
        entries: Vec<Vec<KeyEntry<'de>>>,
        path: Vec<&'de str>,
    },
}
struct Deser<'de> {
    doc: &'de FlatDoc,
    tables: Vec<TableEntry<'de>>,
    state: State<'de>,
}

impl<'de> Deser<'de> {
    fn new(doc: &'de FlatDoc) -> Self {
        Self {
            tables: tables(doc),
            doc,
            state: State::Table { entry: 0 },
        }
    }
    fn stxt(&self, idx: usize) -> &'de str {
        let s = self.doc.spans[idx];
        &self.doc.source[s.start as usize..s.end as usize]
    }
    fn dstr(raw: &str, kind: SpanKind) -> Result<String, Error> {
        ...
        let mut o = String::with_capacity(inner.len());
            ...
            match c.next() {
                ...
                Some('u') => {
                    ...
                    let cp = u16::from_str_radix(&h, 16)
                        .map_err(|_| Error::Message("bad \\u".into()))?;
                    o.push(char::from_u32(cp as u32).unwrap_or('\u{FFFD}'));
                }
                Some('U') => {
                    let h: String = c.by_ref().take(8).collect();
                    let cp = u32::from_str_radix(&h, 16)
                        .map_err(|_| Error::Message("bad \\U".into()))?;
                    o.push(char::from_u32(cp).unwrap_or('\u{FFFD}'));
                }
...
fn cln<'de>(doc: &'de FlatDoc, idx: usize) -> &'de str {
    let s = doc.spans[idx];
    let b = doc.source.as_bytes();
    let mut st = s.start as usize;
    let mut e = s.end as usize;
    while st < e && matches!(b[st], b' ' | b'\t') {
        st += 1
    }
    while e > st && matches!(b[e - 1], b' ' | b'\t') {
        e -= 1
    }
    if e > st + 1 && matches!(b[st], b'"' | b'\'') && b[st] == b[e - 1] {
        st += 1;
        e -= 1
    }
    &doc.source[st..e]
}
fn ival(k: SpanKind) -> bool {
    matches!(
        k,
        SpanKind::Integer
            | SpanKind::Float
...
fn tables<'de>(doc: &'de FlatDoc) -> Vec<TableEntry<'de>> {
    let mut ts: Vec<TableEntry<'_>> = vec![TableEntry {
        path: vec![],
        keys: vec![],
        aot_entries: vec![],
    }];
            ...
            SpanKind::ArrayOpen | SpanKind::ArrayTableOpen => {
                ...
                if in_aot && !aot_buf.is_empty() {
                    if let Some(ei) = ts.iter().position(|t| t.path == cur) {
                        ts[ei]
                            .aot_entries
                            .push(core::mem::replace(&mut aot_buf, vec![]));
                    }
                }
...
fn wlk_arr(spans: &[Span], open: usize) -> Vec<(usize, SpanKind)> {
    let mut e = vec![];
    let mut i = open + 1;
    let mut d = 1;
    while i < spans.len() && d > 0 {
        match spans[i].kind {
...
fn wlk_inl<'de>(spans: &[Span], src: &'de str, open: usize) -> Vec<(KeyEntry<'de>, usize)> {
    let mut p = vec![];
    let mut i = open + 1;
    let mut d = 1;
    let mut ck: Option<&str> = None;
    while i < spans.len() && d > 0 {
        match spans[i].kind {
            ...
            SpanKind::InlineTableClose | SpanKind::ArrayClose => {
                d -= 1;
                if d == 0 {
                    break;
                }
            ...
            SpanKind::BareKey | SpanKind::BasicString | SpanKind::LiteralString if d == 1 => {
                if ck.is_none() {
                    ck = Some(&src[spans[i].start as usize..spans[i].end as usize])
                }
                i += 1
            }
            SpanKind::Equals if d == 1 => {
                i += 1;
                while i < spans.len() {
                    if ival(spans[i].kind) {
                        if let Some(k) = ck.take() {
                            let cl = if (k.starts_with('"') && k.ends_with('"'))
...
// ---- Deserializer ----
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deser<'de> {
    type Error = Error;
    fn deserialize_any<V: Visitor<'de>>(self, v: V) -> Result<V::Value, Error> {
        match self.state {
            State::Table { entry } => v.visit_map(T {
                de: self,
                entry,
                pos: 0,
            }),
            State::Value { idx } => dval(self, idx, v),
            State::Array { ref elements } => {
...
fn dval<'de, V: Visitor<'de>>(de: &mut Deser<'de>, idx: usize, v: V) -> Result<V::Value, Error> {
    let s = de.doc.spans[idx];
    match s.kind {
        SpanKind::Boolean => v.visit_bool(de.stxt(idx) == "true"),
        SpanKind::Integer => {
            let t = de.stxt(idx);
            let val = if t.starts_with("0x") || t.starts_with("0X") {
                i64::from_str_radix(&t[2..], 16)
            } else if t.starts_with("0o") || t.starts_with("0O") {
                i64::from_str_radix(&t[2..], 8)
            } else if t.starts_with("0b") || t.starts_with("0B") {
                i64::from_str_radix(&t[2..], 2)
            } else {
                t.replace('_', "").parse::<i64>()
            }
            .map_err(|_| Error::Message(format!("invalid integer: {t}")))?;
            v.visit_i64(val)
        }
        SpanKind::Float => {
            let t = de.stxt(idx);
            if t == "inf" || t == "+inf" {
                return v.visit_f64(f64::INFINITY);
            }
            if t == "-inf" {
                return v.visit_f64(f64::NEG_INFINITY);
            }
            if t == "nan" || t == "+nan" || t == "-nan" {
                return v.visit_f64(f64::NAN);
            }
            v.visit_f64(
                t.parse()
                    .map_err(|_| Error::Message(format!("invalid float: {t}")))?,
            )
        }
        SpanKind::Datetime => {
            let text = de.stxt(idx);
            let dt = text
                .parse::<toml_datetime::Datetime>()
                .map_err(|_| Error::Message(format!("invalid datetime: {text}")))?;
            v.visit_map(toml_datetime::de::DatetimeDeserializer::<Error>::new(dt))
        }
        SpanKind::BasicString
        | SpanKind::LiteralString
        | SpanKind::MlBasicString
        | SpanKind::MlLiteralString => {
            let raw = de.stxt(idx);
            v.visit_str(&Deser::dstr(raw, s.kind)?)
        }
        SpanKind::ArrayOpen => {
            let el = wlk_arr(&de.doc.spans, idx);
            let sv = core::mem::replace(&mut de.state, State::Value { idx: 0 });
            let r = v.visit_seq(A {
                de,
                elements: el,
                pos: 0,
        ...
        SpanKind::InlineTableOpen => {
            let pr = wlk_inl(&de.doc.spans, &de.doc.source, idx);
            let sv = core::mem::replace(&mut de.state, State::Value { idx: 0 });
            let r = v.visit_map(I {
                de,
                pairs: pr,
...
impl<'de, 'a> MapAccess<'de> for T<'a, 'de> {
    ...
    fn next_key_seed<K: DeserializeSeed<'de>>(&mut self, s: K) -> Result<Option<K::Value>, Error> {
        ...
        if self.pos >= self.de.tables[self.entry].keys.len() {
            ...
            let sub = self
                ...
                .tables
                .iter()
                .position(|t| t.path.len() == cp.len() + 1 && t.path.starts_with(cp));
            if let Some(idx) = sub {
                self.pos = usize::MAX; // mark as yielded
                return s
                    .deserialize((*self.de.tables[idx].path.last().unwrap()).into_deserializer())
                    .map(Some);
            }
            return Ok(None);
        }
        s.deserialize(
            self.de.tables[self.entry].keys[self.pos]
                .key
                .into_deserializer(),
        )
        .map(Some)
    ...
    fn next_value_seed<V: DeserializeSeed<'de>>(&mut self, seed: V) -> Result<V::Value, Error> {
        if self.pos == usize::MAX {
            ...
            let _save = if aot.is_empty() {
                core::mem::replace(&mut self.de.state, State::Table { entry: sub })
            } else {
                let p = self.de.tables[sub].path.clone();
                core::mem::replace(
                    &mut self.de.state,
                    State::AoT {
                        entries: aot,
            ...
            self.de.state = State::Table { entry: self.entry };
            return r;
        }
        let ke = self.de.tables[self.entry].keys[self.pos];
        self.pos += 1;
...
impl<'de, 'a> SeqAccess<'de> for A<'a, 'de> {
    type Error = Error;
    fn next_element_seed<T: DeserializeSeed<'de>>(
        ...
        let (idx, _) = self.elements[self.pos];
        self.pos += 1;
        let sv = core::mem::replace(&mut self.de.state, State::Value { idx });
        let r = s.deserialize(&mut *self.de);
        self.de.state = sv;
        r.map(Some)
    }
}
...
impl<'de, 'a> MapAccess<'de> for I<'a, 'de> {
    type Error = Error;
    fn next_key_seed<K: DeserializeSeed<'de>>(&mut self, s: K) -> Result<Option<K::Value>, Error> {
        if self.pos >= self.pairs.len() {
            return Ok(None);
        }
        s.deserialize(self.pairs[self.pos].0.key.into_deserializer())
            .map(Some)
    }
    fn next_value_seed<V: DeserializeSeed<'de>>(&mut self, seed: V) -> Result<V::Value, Error> {
        let ke = self.pairs[self.pos];
        self.pos += 1;
        let sv = core::mem::replace(
            &mut self.de.state,
            State::Value {
                idx: ke.0.value_idx,
            },
...
impl<'de, 'a> SeqAccess<'de> for AoT<'a, 'de> {
    type Error = Error;
    fn next_element_seed<T: DeserializeSeed<'de>>(
        &mut self,
        s: T,
        ...
        let keys = self.entries[self.pos].clone();
        self.pos += 1;
        let sv = core::mem::replace(&mut self.de.state, State::Table { entry: 0 });
        let ol = self.de.tables.len();
        self.de.tables.push(TableEntry {
            path: self.path.clone(),
            keys,
            aot_entries: vec![],
        });
        self.de.state = State::Table { entry: ol };
        let r = s.deserialize(&mut *self.de);
        self.de.tables.pop();
        self.de.state = sv;
        r.map(Some)
